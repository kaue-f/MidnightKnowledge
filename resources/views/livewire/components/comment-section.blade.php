<div class="flex flex-col w-full py-6 px-8 gap-4">
                        ...
                        <small class="w-full text-end text-xs opacity-80 font-light">
                            Limite: <span
                                :class="{
                                    ...
                                    'opacity-80': ($wire.comment.length || 0) <= 500
                                }"
    ...
    <div class="flex flex-col gap-4" x-data="{ loaded: false }" x-init="window.onscroll = function() {
        ...
        @foreach ($posts as $post)
            <div
                class="flex flex-row rounded-sm bg-base-300 pt-2.5 pb-1.5 px-4 gap-5 border-[1.5px] border-neutral border-b-[1.5px] border-b-secondary">
